...
from .common import Elastic_Net, Residual


class BindTransformerConfigTensorDict(PretrainedConfig):
    """
...
class BindTransformerModelTensorDict(PreTrainedModel):
    """
    config_class: The corresponding cofiguration class of BindTransformerModel.
    """

    config_class = BindTransformerConfigTensorDict

    def __init__(self, config: BindTransformerConfigTensorDict) -> None:
        """
        config: An instance of the configuration class.
        """
        super().__init__(config)

        self.second_encoder = TensorDictSequential(
            TensorDictModule(
                lambda second_ids: second_ids != 0,
                in_keys=["second_ids"],
                out_keys=["second_mask"],
            ),
            TensorDictModule(
                lambda second_mask: einsum(
                    second_mask, second_mask, "b s1, b s2 -> b s1 s2"
                ),
                in_keys=["second_mask"],
                out_keys=["second_mask_outer"],
            ),
            TensorDictModule(
                nn.Embedding(
                    config.second_vocab,
                ...
                in_keys=["second_ids"],
                out_keys=["second_embs"],
            ),
            *sum(
                [
                    [
                        TensorDictModule(
                            lambda second_embs: second_embs.clone(),
                            in_keys=["second_embs"],
                            out_keys=["second_embs_residual"],
                        ),
                        TensorDictModule(
                        ...
                        TensorDictModule(
                            MultiHeadAttention(
                                ...
                                q_proj=EinMix(
                                    ...
                                    bias_shape="nhd",
                                    d=config.dim_emb,
                                    nhd=config.num_heads * config.dim_heads,
                                ),
                                k_proj=EinMix(
                                    "b s d -> b s nhd",
                                    weight_shape="d nhd",
                                ...
                                pos_embeddings=RotaryPositionalEmbeddings(
                                    dim=config.dim_heads,
                                    max_seq_len=config.max_num_tokens,
                                ),
                                max_seq_len=config.max_num_tokens,
                                is_causal=False,
                                attn_dropout=config.dropout,
                            ),
                            in_keys=["second_embs", "second_embs", "second_mask_outer"],
                            out_keys=["second_embs"],
                        ),
                        TensorDictModule(
                            lambda second_embs, second_embs_residual: second_embs
                            + second_embs_residual,
                            in_keys=["second_embs", "second_embs_residual"],
                            out_keys=["second_embs"],
                        ),
                        TensorDictModule(
                            Residual(
            ...
            selected_out_keys=["second_embs", "second_mask"],
        )
    ...
    def forward(
        ...
        # 二级结构编码
        second_embs, second_mask = self.second_encoder(second_ids=second_ids)
        # # DNA编码
