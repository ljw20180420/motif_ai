...
class BindTransformerConfig(PretrainedConfig):
    ...
    def __init__(
        self,
        protein_animo_acids_vocab_size: int = None,
        protein_secondary_structure_vocab_size: int = None,
        protein_coarse_grained_size: int = None,
        protein_max_position_embeddings: int = None,
        DNA_vocab_size: int = None,
        DNA_max_position_embeddings: int = None,
...
class BindTransformerModel(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.protein_embedding_layer = EmbeddingLayerNormDropOut(
        ...
        self.second_embedding_layer = EmbeddingLayerNormDropOut(
    ...
    def forward(
        ...
        """
        protein_ids, second_ids = self.accession_input(
            protein_ids, second_ids, DNA_ids, bind
        )

        # 蛋白质氨基酸编码+二级结构编码。
        protein_embeddings = self.protein_embedding_layer(
            protein_ids,
        ) + self.second_embedding_layer(
            second_ids,
        )
        ...
        protein_sinusoidal_pos = self.protein_sinusoidal(protein_embeddings.shape[:-1])[
            None, None, :, :
        ]
        # DNA位置编码 [batch_size, DNA_length] -> [batch_size, num_heads, DNA_length, embedding_size_per_head]
        DNA_sinusoidal_pos = self.DNA_sinusoidal(DNA_embeddings.shape[:-1])[
            None, None, :, :
        ]
        # 根据mask token（0）计算attention mask
        protein_mask = (
            protein_ids.view(
                protein_ids.shape[0], -1, self.config.protein_coarse_grained_size
            )
            .any(dim=2)
            .to(self.dtype)
        )
        protein_mask = (1.0 - protein_mask) * torch.finfo(self.dtype).min
        protein_mask = protein_mask[:, None, None, :]

        DNA_mask = (1.0 - DNA_ids.to(torch.bool).to(self.dtype)) * torch.finfo(
            self.dtype
        ).min
        DNA_mask = DNA_mask[:, None, None, :]
        # 计算蛋白质encoder
        for layer in self.encoder:
            protein_embeddings = layer(
                protein_embeddings, protein_mask, protein_sinusoidal_pos
            )
        # 计算DNA decoder
    ...
    def accession_input(
        ...
        # 检查氨基酸和二级结构长度一致
        assert (
            protein_ids.shape[-1] == second_ids.shape[-1]
        ...
        padding_size = (
            self.config.protein_coarse_grained_size
            - (protein_ids.shape[-1] - 1) % self.config.protein_coarse_grained_size
            - 1
        )
        protein_ids = F.pad(protein_ids, (0, padding_size))
        second_ids = F.pad(second_ids, (0, padding_size))
        assert (
            protein_ids.shape[-1] <= self.config.protein_max_position_embeddings
        ), "protein is too long"
        # 检查DNA是否太长
        assert DNA_ids.shape[-1] <= self.config.DNA_max_position_embeddings

        return protein_ids, second_ids
